...
if (isset($guesteditoruse)) {
    ...
    if ($xoopsUser) {
        $editor = LexikonUtility::getWysiwygForm(_MD_LEXIKON_DEFINITION, 'definition', $definition, 15, 60);
        if ($definition == _MD_LEXIKON_WRITEHERE) {
            $editor->setExtra('onfocus="this.select()"');
        }
        $sform->addElement($editor, true);
        ...
        $def_block = new XoopsFormDhtmlTextArea(_MD_LEXIKON_DEFINITION, 'definition', $definition, 15, 60);
