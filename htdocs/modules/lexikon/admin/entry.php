...
function entryEdit($entryID = '')
{
    ...
    // Author selector
/* pas utile, puisqu'on recupere le pseudo de l'utilisateur
   donc aucun risque que l'on mette unautre pseudo
    ob_start();
...
*/
    ...
    // Initial selector
/* JJD - pas utile l'initial est calculer automatiquement au memen d'enregistrer l'entree
    ob_start();
    lx_getinit((int)$init);
...
function entrySave($entryID = '')
{
    ...
    $definition = $myts->addSlashes($myts->censorString($_POST['definition']), $allowimage = 1);
