...
//JJDai utilisation du form en edition en plus d new
...
//     $op = trim('edit');
// }
$entryID = (isset($request['entryID']) ? $request['entryID'] : 0);

// $tr= print_r($request,true);
// echo "<hr><pre>{$tr}</pre><hr>";
// echo "===>ici entryID = {$entryID}<hr>";
// exit;
...
switch ($op) {
    case 'post':
        ...
        if ($entryID == 0){
          $isNewEntry = true;
          $entryIDFind = LexikonUtility::getTermByName($term, $xoopsDB->prefix('lxentries'));
          if ($entryIDFind > 0 ){
              redirect_header('entry.php?entryID='. $entryIDFind, 2, sprintf(_MD_LEXIKON_ITEMEXISTS, $term) );
          }
/*
...
*/
          ...
          $isNewEntry = false;
          $sql = 'UPDATE ' . $xoopsDB->prefix('lxentries')
...
//echo "<hr>{$sql}<hr>";exit;
        ...
        if ($result) {
            ...
            if ($autoapprove == 1) {
//exit ("===>stop ici");
                //JJDai retour au terme qui vient d'etre modifié
                $url = "entry.php?entryID={$entryID}";
                redirect_header($url , 2, _MD_LEXIKON_RECEIVEDANDAPPROVED);
...
//exit;
            ...
            $notifypub  = 1;
            $term       = $termino;
            $definition = _MD_LEXIKON_WRITEHERE;
            $ref        = '';
            $url        = '';
        }
//echo "<hr>===> entryID = {$entryID} | nbRows = " . $xoopsDB->getRowsNum($result) . "<br>term = $term<br>definition = {$definition}<hr>";
