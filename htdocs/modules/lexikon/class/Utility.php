...
public static function getServiceLinks($variable)
{
    ...
    if ($xoopsUser) {
        ...
        */
            //JJDai - ajot de la modication d'une entre dans le front office, plus cohérent
            $srvlinks .= "<a TITLE=\""
                             . _EDIT
                             . "\" href=\"submit.php?op=post&entryID="  // http://cds.jubile.fr/modules/lexikon/submit.php
                             . $variable['id']
                             . "\" ><img src=\""                        // "\" target=\"_blank\"><img src=\""
                             . $pathIcon16 . "/edit.png\"   border=\"0\" alt=\""
                             . _MD_LEXIKON_EDITTERM
                             . "\" width=\"16\" height=\"16\"></a>&nbsp;<a TITLE=\""
                             . _DELETE
...
//JJDai - remplaceùent de la fonction crete_function depreciée par une fonction anonyme
    $text = preg_replace_callback("&#(\d+)&", function($matches) {return  chr($matches[1]);}, $text);



//    $text = preg_replace_callback("&#(\d+)&", function($matches) use ($text) {return  chr(\$matches[1]);} );
...
public static function getTermByName($term, $table)
{
    global $xoopsDB;
    $sql    = sprintf('SELECT COUNT(*), entryID FROM %s WHERE term = %s', $table, $xoopsDB->quoteString(addslashes($term)));
    $result = $xoopsDB->query($sql);
    list($count, $entryID) = $xoopsDB->fetchRow($result);
