...
if (!$init) {
    ...
    //JJDai correction de l'ordre de la liste des champs
    $lstFields = "entryID,categoryID,term,init,definition,ref,url,uid,submit,datesub,counter,html,smiley,xcodes,breaks,block,offline,comments";
    $queryA  = 'SELECT ' . $lstFields . ' FROM ' . $xoopsDB->prefix('lxentries') . " WHERE offline = '0' AND submit = '0' " . $catperms . ' ORDER BY term ASC';
    $resultA = $xoopsDB->query($queryA, $xoopsModuleConfig['indexperpage'], $start);
    ...
    */
    if ($init == _MD_LEXIKON_OTHER) {
      ...
      $filterInit = " AND init = '{$init}' AND init != '#'" ;
    }

    $lstFlds = "entryID, categoryID, term, definition, url, uid, html, smiley, xcodes, breaks, comments";
    $queryB  = 'SELECT ' . $lstFlds . ' FROM ' . $xoopsDB->prefix('lxentries')
               . " WHERE submit ='0' AND offline = '0'" . $filterInit
               . $catperms
               . '  ORDER BY term ASC';
    $resultB = $xoopsDB->query($queryB, $xoopsModuleConfig['indexperpage'], $start);
    // /JJDai
    ...
    //--------------------------------------------
    $allentries = $xoopsDB->query('SELECT entryID FROM ' . $xoopsDB->prefix('lxentries')
                . " WHERE submit ='0' AND offline = '0' " . $filterInit . $catperms
                . '  ORDER BY term ASC ');
    //--------------------------------------------
    /*
    if ($init == _MD_LEXIKON_OTHER) {
    ...
    */
    ...
    while (list($entryID, $categoryID, $term, $definition, $url, $uid, $html, $smiley, $xcodes, $breaks, $comments) = $xoopsDB->fetchRow($resultB)) {
        $eachentry        = array();
        $xoopsModule      = XoopsModule::getByDirname('lexikon');
        $eachentry['dir'] = $xoopsModule->dirname();
...
//JJDai - ajout du formulaire de recheche sur le formulaire "letter.tpl"
